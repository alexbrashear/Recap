...
class SentPostcardsViewModel: SentPostcardsViewModelProtocol {
    ...
    init(sentPostcards: [Postcard]) {
        self.sentPostcards = sentPostcards
    }
    ...
    func cellConfiguration(for indexPath: IndexPath) -> SentPostcardCellConfiguration {
        ...
        return SentPostcardCellConfiguration(name: sentPostcard.to.name, expectedDelivery: sentPostcard.expectedDeliveryDate, thumbnailURL: sentPostcard.imageThumbnails.small)
    }
}
